...
export function AcssImportWizard({
  current,
  onApply,
}: {
  ...
  const buildPatch = (): Partial<AutofuseTokens> => {
    ...
    if (include.radius && json.radius)
      patch.radius = { ...current.radius, ...json.radius };
    if (include.shadows && json.shadows)
      patch.shadows = { ...current.shadows, ...json.shadows };
    return patch;
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <label style={{ fontSize: "var(--af-text-sm)" }}>
          Upload JSON{" "}
          <input
            type="file"
          ...
          {["colors", "typography", "spacing", "radius", "shadows"].map((k) => (
            <label key={k} style={{ display: "block" }}>
              <input
                ...
                checked={include[k]}
                onChange={(e) =>
                  setInclude({ ...include, [k]: e.target.checked })
                }
              />{" "}
              {k}
            </label>
          ))}
          <div style={{ marginTop: "0.5rem", fontSize: "var(--af-text-sm)" }}>
            Change count: {changes}
          </div>
          {include.colors && colorKeys.length > 0 && (
            <div style={{ marginTop: "0.5rem" }}>
              <div style={{ fontWeight: 600, marginBottom: "0.25rem" }}>
                Colors preview (select per-shade)
              </div>
                  ...
                  <div key={role}>
                    ...
                    >
                      ...
                      {["50", "100", "300", "500", "700", "900"].map((s) => {
                        ...
                        const changed =
                          incoming !== undefined && incoming !== cur;
                        ...
                        return (
                          <React.Fragment key={s}>
                            ...
                            >
                              <div
                                style={{
                                  ...
                                  background: cur,
                                  border:
                                    "1px solid var(--af-color-neutral-300)",
                                  borderRadius: 3,
                                }}
                              />
                              <code>{cur || "-"}</code>
                            ...
                            <div
                              style={{
                                color: changed
                                  ? "var(--af-color-warning-500, #b45309)"
                                  : "var(--af-color-neutral-500)",
                              }}
                            >
                              {changed
                                ? d
                                  ? `ΔL ${d.dL} Δa ${d.dA} Δb ${d.dB}`
                                  : "changed"
                                : "same"}
                            </div>
