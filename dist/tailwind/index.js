...
function autofusePlugin(_opts = {}) {
  ...
  return u(
    function({ addBase, addUtilities: twAddUtilities, addVariant }) {
      const layer = _opts.layer || "utilities";
      const addUtilities = (obj) => twAddUtilities(obj, { layer });
      const rootColorVars = {};
      ...
      const screenUtils = {};
      Object.entries(tokens.breakpoints).forEach(
        ([name, px]) => {
      ...
      addUtilities({
        ".af-bg-gradient": {
          backgroundImage: "linear-gradient(var(--af-grad-direction, to bottom), var(--af-grad-from, transparent), var(--af-grad-via, initial), var(--af-grad-to, transparent))"
        },
        ".af-bg-gradient-radial": {
          backgroundImage: "radial-gradient(var(--af-grad-shape, circle) var(--af-grad-size, farthest-corner) at var(--af-grad-position, center), var(--af-grad-from, transparent), var(--af-grad-via, initial), var(--af-grad-to, transparent))"
        },
        ".af-bg-gradient-conic": {
          backgroundImage: "conic-gradient(from var(--af-grad-angle, 0deg) at var(--af-grad-position, center), var(--af-grad-from, transparent), var(--af-grad-via, initial), var(--af-grad-to, transparent))"
        },
        ...
        ".af-bg-gradient-rainbow": {
          backgroundImage: "linear-gradient(45deg, #ff0000, #ff8000, #ffff00, #80ff00, #00ff00, #00ff80, #00ffff, #0080ff, #0000ff, #8000ff, #ff00ff, #ff0080)"
        },
      ...
      addUtilities({
        ...
        ".af-select": {
          ...
          background: `white url("data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3E%3Cpath stroke='%236B7280' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3E%3C/svg%3E") no-repeat right 0.5rem center/1.5rem 1.5rem`,
          color: "var(--af-color-neutral-900)",
          fontSize: "var(--af-text-base)",
          lineHeight: "1.5",
          cursor: "pointer",
      ...
      addUtilities({
        ...
        ".af-overlay": {
          ...
          "&::before": {
            ...
            position: "absolute",
            inset: "0",
            background: "var(--af-overlay-bg, linear-gradient(var(--af-overlay-angle, 180deg), var(--af-overlay-from, rgba(0,0,0,0.4)), var(--af-overlay-to, rgba(0,0,0,0))))",
        ...
        // Radial overlays
        ".af-overlay-radial": {
          "--af-overlay-bg": "radial-gradient(circle at var(--af-overlay-position, center), var(--af-overlay-from, rgba(0,0,0,0.4)), var(--af-overlay-to, rgba(0,0,0,0)))"
        },
        ...
        ".af-overlay-spotlight": {
          "--af-overlay-bg": "radial-gradient(circle at var(--af-overlay-position, center), transparent 20%, rgba(0,0,0,0.8) 70%)"
        },
        ".af-overlay-vignette": {
          "--af-overlay-bg": "radial-gradient(ellipse at center, transparent 30%, rgba(0,0,0,0.5) 100%)"
        },
        // Blend modes
        ...
        ".af-bg-overlay": {
          ...
          "&::before": {
            ...
            position: "absolute",
            inset: "0",
            background: "linear-gradient(var(--af-overlay-angle, 180deg), var(--af-overlay-from, rgba(0,0,0,0.35)), var(--af-overlay-to, rgba(0,0,0,0)))",
            pointerEvents: "none",
            zIndex: "1"
          },
          "& > *": {
